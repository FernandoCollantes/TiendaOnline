...
import { obtenerTienda, obtenerProductosVistos, guardarProductosVistos } from '../utils/localStorage.js';
...
function inicializarProducto() {
    ...
    if (!productoId) {
        productDetail.innerHTML = '<p class="text-center">Producto no encontrado</p>';
        return;
    }
...
function cargarProducto(productoId) {
    ...
    if (!tienda || !tienda.productos) {
        productDetail.innerHTML = '<p class="text-center">Error al cargar el producto</p>';
        return;
    }
...
function mostrarDetalleProducto(producto) {
    ...
    const html = `
        ...
        <div class="product-detail-info">
            ...
            <div class="product-detail-price">€${producto.precio.toFixed(2)}</div>
            ...
            <div class="quantity-controls">
                ...
                <div class="quantity-selector">
                    <button class="quantity-btn" id="btnDecrease" ${cantidadSeleccionada <= 1 ? 'disabled' : ''}>-</button>
                    <span class="quantity-value" id="quantityValue">${cantidadSeleccionada}</span>
                    <button class="quantity-btn" id="btnIncrease" ${cantidadSeleccionada >= producto.stock ? 'disabled' : ''}>+</button>
                </div>
            </div>
            
            <div class="product-actions">
                <button class="btn btn-primary btn-add-to-cart" id="btnAddToCart" ${producto.stock === 0 ? 'disabled' : ''}>
...
function configurarEventoCarrito() {
    ...
    btnAddToCart.addEventListener('click', () => {
        ...
        actualizarBadgeCarrito();
        mostrarNotificacion(`✅ ${cantidadSeleccionada} x ${productoActual.nombre} añadido al carrito`);
...
function cargarProductosRelacionados(categoriaId, productoActualId) {
    ...
    if (relacionados.length === 0) {
        productosRelacionados.innerHTML = '<p class="text-center">No hay productos relacionados</p>';
        return;
    }
...
function configurarEventosRelacionados(todosLosProductos) {
    const productCards = document.querySelectorAll('#productosRelacionados .product-card');
    ...
    const addToCartButtons = document.querySelectorAll('#productosRelacionados .btn-add-cart');
